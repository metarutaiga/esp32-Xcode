...
static void aes_gctr(const u8 *icb, const u8 *x, size_t xlen, u8 *y)
{
    size_t i, n, last;
    ...
    /* Full blocks */
    for (i = 0; i < n; i++) {
        aes_hal_transform_block(cb, ypos);
        xor_block(ypos, xpos);
        xpos += AES_BLOCK_SIZE;
        ypos += AES_BLOCK_SIZE;
        inc32(cb);
    ...
    if (last) {
        /* Last, partial block */
        aes_hal_transform_block(cb, tmp);
        for (i = 0; i < last; i++)
            *ypos++ = *xpos++ ^ tmp[i];
    }
}
...
static void aes_gcm_gctr(const u8 *J0, const u8 *in, size_t len, u8 *out)
{
               ...
               const u8 *aad, size_t aad_len, const u8 *tag, u8 *plain)
{
    ...
    aes_gcm_gctr(J0, crypt, crypt_len, plain);

    aes_gcm_ghash(H, aad, aad_len, crypt, crypt_len, S);

    /* T' = MSB_t(GCTR_K(J_0, S)) */
    aes_gctr(J0, S, sizeof(S), T);
